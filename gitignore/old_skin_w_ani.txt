...
function loadBones(bones, texturewidth, textureheight, material) {
  ...
  // First pass: create all bones and store raw data
  for (const bone of bones) {
    ...
    if (bone.cubes) {
      bone.cubes.forEach((cube) => {
        ...
        box_mesh.position.sub(new THREE.Vector3(...bone.pivot));
  ...
  // Second pass: apply hierarchy and relative pivot
  for (const name in boneMap) {
    ...
    if (data.parent && boneMap[data.parent]) {
      ...
      // Adjust pivot to be relative to parent
...
// Helper function to recursively apply animation rotations to bones and their children
function applyAnimationRecursively(object3d, animBones, parentHasAnim = false) {
  ...
  if (hasAnim) {
    ...
    // Propagate parent's rotation to child bones (optional)
    // Or just keep child's rotation as is, since child is already parented and moves with parent
  }
...
export function generateSkinModel(texture, geometry, scale = 1, skin, animationDefs) {
  ...
  const bones = loadBones(geometry.bones, texture.image.width, texture.image.height, material);
  ...
  // Removed scaling to avoid size issues
    ...
    const animationDefs = {
      ...
      "animation.player.sneaking.inverted": {
        ...
        bones: {
          ...
          head: { position: [0.0, -1.0, 0.0], rotation: [-28.0, 0.0, 0.0] },
          leftarm: { rotation: [-5.7, 0.0, 0.0] },
          leftleg: { rotation: [28.0, -0.1, -0.1] },
          rightarm: { rotation: [-5.7, 0.0, 0.0] },
          rightleg: { rotation: [28.0, 0.1, 0.1] },
          root: { position: [0.0, 1.25, -8.0], rotation: [-28.0, 0.0, 0.0] },
        },
      },
      "animation.player.base_pose.upside_down": {
        loop: true,
        bones: {
          head: { position: [0.0, 8.0, 0.0], rotation: [0.0, 0.0, 180.0] },
          body: { position: [0.0, 8.0, 0.0], rotation: [0.0, 0.0, 180.0] },
          leftarm: { position: [-5, 10, 0], rotation: [0.0, 0.0, 180.0] },
          rightarm: { position: [5, 10, 0], rotation: [0.0, 0.0, 180.0] },
          rightLeg: { rotation: [0, 0, 180], position: [2, 20, 0] },
          leftLeg: { rotation: [0, 0, 180], position: [-2, 20, 0] },
        },
      },
